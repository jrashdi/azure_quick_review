...
const (
	credNameAzureCLI = "SafeAzureCLICredential"
...
// SafeAzureCLICredentialOptions contains optional parameters for SafeAzureCLICredential.
type SafeAzureCLICredentialOptions struct {
...
// init returns an instance of SafeAzureCLICredentialOptions initialized with default values.
func (o *SafeAzureCLICredentialOptions) init() {
	if o.tokenProvider == nil {
		o.tokenProvider = defaultTokenProvider()
	}
}

// SafeAzureCLICredential authenticates as the identity logged in to the Azure CLI.
type SafeAzureCLICredential struct {
	tokenProvider azureCLITokenProvider
	tenantID      string
	lock          sync.Mutex
}

// NewSafeAzureCLICredential constructs an SafeAzureCLICredential. Pass nil to accept default options.
func NewSafeAzureCLICredential(options *SafeAzureCLICredentialOptions) (*SafeAzureCLICredential, error) {
	cp := SafeAzureCLICredentialOptions{}
	if options != nil {
		cp = *options
	}
	cp.init()
	return &SafeAzureCLICredential{
...
func (c *SafeAzureCLICredential) GetToken(ctx context.Context, opts policy.TokenRequestOptions) (azcore.AccessToken, error) {
...
func (c *SafeAzureCLICredential) authenticate(ctx context.Context, resource string) (azcore.AccessToken, error) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *SafeAzureCLICredential) createAccessToken(tk []byte) (azcore.AccessToken, error) {
	t := struct {
...
var _ azcore.TokenCredential = (*SafeAzureCLICredential)(nil)
